...
bool ThreadReadWriteLock::SetReadLock()
{
    //pthread_rwlock_unlock() //return true;
     int rc = pthread_rwlock_tryrdlock(&readWriteLock);  //on read mode lock readwritelock 
    if (rc == 0)  
        return true;  
    else if (rc == EBUSY)  
        return false;  
...
bool ThreadReadWriteLock::SetWriteLock()
{
	//return  pthread_rwlock_wrlock(readWriteLock);
	 int rc = pthread_rwlock_trywrlock(&readWriteLock);   //on write mode lock readwritelock
    if (rc == 0)  
        return true;  
    else if (rc == EBUSY)  
        return false;  
